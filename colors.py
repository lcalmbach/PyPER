...
# not used yet
LINEAR_COLORS_PALETTES = ['Blues256', 'Cividis256', 'Greens256', 'Inferno256', 'Purples256', 'Reds256']


def get_num_colors(palette_name: str):
    return len(plts.all_palettes[palette_name])


def get_colors(palette_name: str, color_num):
...
def user_input_palette(title, plt, num):
    ...
    color_num = st.number_input('Number of colors', min_value = 2, max_value = max_num, value = num)
    cols = st.columns(2)
    with cols[0]:
