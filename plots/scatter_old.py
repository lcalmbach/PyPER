...
class Scatter:
    ...
    def add_correlation(self, p, df):
        ...
        p.line(
            ...
            df["y"],
            line_width=2,
            color=self.cfg["corr_line_color"],
            line_dash=self.cfg["corr_line_pattern"],
        )
        return p, corr_coeff
    ...
    def get_plot(self):
        def init_plot(x_col, y_col):
            ...
            if self.cfg["axis_auto"] == False:
                plot.x_range = Range1d(
                    float(self.cfg["x_axis_min"]), float(self.cfg["x_axis_max"])
                )
                plot.y_range = Range1d(
                    float(self.cfg["y_axis_min"]), float(self.cfg["y_axis_max"])
                )
        ...
        if self.cfg["show_corr_line"]:
            plot, corr_coeff = self.add_correlation(plot, self.data[[x_col, y_col]])
            p_val = 0.05
            sign_result = (
                "Correlation is statisticically significant"
                if corr_coeff[3] < p_val
                else "Correlation is not statisticically significant"
            )
