...
class Time_series:
    ...
    def add_markers(self, p, df):
        p.circle(
            ...
            fill_color=self.cfg["fill_colors"][0],
    ...
    def get_plot(self):
        ...
        if not self.cfg["y_axis_auto"]:
            ...
            if self.cfg["y_axis_tick_interval"] > 0:
                p.yaxis.ticker = FixedTicker(
                    ticks=get_ticks(
                        self.cfg["y_axis_tick_interval"],
                        (self.cfg["y_axis_start"], self.cfg["y_axis_end"]),
                    )
                )
        if not self.cfg["time_axis_auto"]:
            t_min = datetime(
                self.cfg["time_axis_start"].year,
                self.cfg["time_axis_start"].month,
                self.cfg["time_axis_start"].day,
            )
            t_max = datetime(
                self.cfg["time_axis_end"].year,
                self.cfg["time_axis_end"].month,
                self.cfg["time_axis_end"].day,
            )
            p.x_range = Range1d(t_min, t_max)

        palette_len = 4 if len(self.cfg["parameters"]) <= 4 else 9
        color = itertools.cycle(
            bpal.all_palettes[self.cfg["palette"]][palette_len]
        )  # helper.color_gen(self.cfg['palette'], 10)
        i = 0
        legend_items = []
        ...
        if self.cfg["show_percentile_band"] > 0:
            ...
            box = BoxAnnotation(
                bottom=bottom,
                top=top,
                fill_color=self.cfg["pct_band_color"],
                fill_alpha=self.cfg["pct_band_alpha"],
            )
            p.add_layout(box)
        ...
        if "hlines" in self.cfg:
            for lin in self.cfg["hlines"]:
                hline = Span(
                    location=lin["location"],
                    dimension="width",
                    line_color=lin["color"],
                    line_width=lin["width"],
                )
                p.renderers.extend([hline])
        legend = Legend(items=legend_items, location=(0, 0), click_policy="hide")
